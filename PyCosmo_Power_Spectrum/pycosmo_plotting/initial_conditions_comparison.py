...
def plot_phase_space_diagrams(ngenic_results, pycosmo_results, num_bins=100):
    ...
    for i, column_name in enumerate(coord_labels):
        plot_comparison_histograms(
            ...
            filename=f"{column_name}_initial_coords.pdf",
            num_bins=num_bins
        )
    ...
    for i, column_name in enumerate(vel_labels, start=3):
        plot_comparison_histograms(
            ...
            filename=f"{column_name}_initial_velocities.pdf",
            num_bins=num_bins
        )

print("Creating histograms of initial conditions...")

ngenic_path = os.path.join(parent_dir, 'initial_conditions/', 'initial_ngenic_conditions.csv')
...
pycosmo_path = os.path.join(parent_dir, 'initial_conditions/', 'initial_pycosmo_conditions.csv')
